...
@implementation HealthKitManagerModel
...
+ (NSArray <HealthKitManagerModel *>*)predicateForSamplesHour {
    ...
    for (int i = 0; i < count; i ++) {
        HealthKitManagerModel *model = [[HealthKitManagerModel alloc] init];
...
#pragma mark - 步数
- (void)getStepFromHealthKitComplate:(void(^)(NSArray <HealthKitManagerModel *> *stepArray))handler {
    HKQuantityType *stepType = [HKObjectType quantityTypeForIdentifier:HKQuantityTypeIdentifierStepCount];
    NSArray *arr = [HealthKitManager predicateForSamplesHour];
    __block NSMutableArray *arrM = [NSMutableArray array];
    for (int i = 0; i < arr.count; i ++) {
        HealthKitManagerModel *model = [arr objectAtIndex:i];
...
- (void)getDietaryEnergyConsumedFromHealthKitWithUnit:(HKUnit *)unit withCompltion:(void (^)(double, NSError *))compltion {
    ...
    [self.healthStore emhk_mostRecentQuantitySampleOfType:stepType predicate:[HealthKitManager predicateForSamplesToday:NO] completion:^(NSArray *results, NSError *error) {
        double sum = 0;
...
- (void)fetchSampleQuantitySumOfSamplesTodayForType:(HKSampleType *)sampleType unit:(HKUnit *)unit isLatest:(BOOL)isLatest completion:(void (^)(double,NSError *))completionHandler {
    NSPredicate *predicate = [HealthKitManager predicateForSamplesToday:isLatest];
...
- (void)fetchQuantitySumOfSamplesTodayForType:(HKQuantityType *)quantityType unit:(HKUnit *)unit completion:(void (^)(double, NSError *))completionHandler {
    NSPredicate *predicate = [HealthKitManager predicateForSamplesToday:NO];
...
#pragma mark-获取时长
- (void)getStepCount:(void(^)(double value,NSString * hours,NSError *error))completion{
...
//                NSLog(@"%f",usersHeight);
                ...
                NSString * dev = quantitySample.sourceRevision.source.name;
                
                if (dev == UIDevice.currentDevice.name) {
...
//            NSLog(@"运动时长：%@小时%@分", @(h), @(m));
            if (h == 0) {
                completion(totleSteps,[NSString stringWithFormat:@"%@分",@(m)],error);
            }else{
                completion(totleSteps,[NSString stringWithFormat:@"%@小时%@分",@(h),@(m)],error);
            }
        }
    }];
